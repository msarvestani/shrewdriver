...
def load_parameters(task):
    print "Using settings for Qbert-2AFC!"
    ...
    #task.sPlusDir = 'Phase1_OriStims/Fig_left/Left_Movie_'
    #task.sMinusDir = 'Phase1_OriStims/Fig_right/Right_Movie_'
    task.sPlusDir = 'version8/Fig_left/Left_Movie_'
    task.sMinusDir = 'version8/Fig_right/Right_Movie_'


    # Stim and task params
    # task.boundary = randint(45,135)     #boundary = 45 [45-135]
    task.boundary = int(180)
    task.deviations = [.1, .5]#[45, 90, 135]#[10, 22.5, 45]
    if not task.useMovie:
        ...
        task.sLeftOrientations = [1, 2, 3, 4, 5, 6]
        task.sRightOrientations = [1, 2, 3, 4, 5, 6]
    ...
    task.timeoutWrongLick = 5  # applies only when guaranteedSPlus is false
    task.initTime = 1

    task.variableDelayMin = 1.0
    task.variableDelayMax = 1.5

    task.gratingDuration = 2#1
    ...
    task.commandStrings[States.SLEFT] = 'acgf sf0.25 tf4 jf0 ja0 gc0.9 px0 py0 sx40 sy40\n'
    #task.commandStrings[States.SRIGHT] = 'as sf0.25 tf4 gc0.9 jf0 ja0 px0 py0 sx999 sy999\n'
    task.commandStrings[States.SRIGHT] = 'acgf sf0.25 tf4 jf0 ja0 gc0.9 px0 py0 sx40 sy40\n'
