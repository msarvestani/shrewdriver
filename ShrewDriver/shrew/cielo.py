...
def load_parameters(task):
    print "Using settings for cielo!"
    ...
    task.sPlusOrientations = [90, 135, 180]
    #task.sPlusOrientations = [45, 90, 135, 180] #Cielo acclimation
    #task.sMinusOrientations = [67.5, 90, 135] # [40]
    #task.sMinusOrientations = [55, 67.5, 90]  # [40]
    #task.sMinusOrientations = [0]
    task.sMinusOrientations = [0] #how many times to display the SMINUS
    ...
    task.timeoutNoResponse = 2
    task.timeoutCorrectReject = 1  # applies only when guaranteedSPlus is false
    task.initTime = 2

    task.variableDelayMin = 2.0
    task.variableDelayMax = 2.25

    task.gratingDuration = 0.5
    task.grayDuration = 2.0 #1.5
    task.rewardPeriod = task.grayDuration  # needs to be no longer than gray duration!

    # Rewards / Hints
    task.rewardBolus = 120 # Microliters
    task.rewardBolusHardTrial = 60  # Microliters
    task.hintBolus = 10  # Microliters
    task.catchChance = 1.0 #0 leads to no reward.. this is counterintuitive?
    task.rewardDelay = 0

    task.hintChance = 0.4  # chance of sending a low reward at the start of the reward period
    ...
    task.commandStrings[States.SMINUS] = 'as sf0.25 tf0 jf2 ja1.5 gc0.9 px0 py0 sx999 sy999\n'
    #task.commandStrings[States.SMINUS] = 'acgf sf0.05 tf0 jf0 ja0 gc0.9 px0 py0 sx40 sy40\n'
    task.commandStrings[States.GRAY] = 'sx0 sy0\n'
    task.commandStrings[States.SPLUS] = 'as sf0.25 tf0 jf2 ja1.5 gc0.9 px0 py0 sx999 sy999\n'
