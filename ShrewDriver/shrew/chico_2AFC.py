...
def load_parameters(task):
    ...
    # Choose whether a grating stim or a stim saved as a movie is used
    task.useMovie = False
    # Choose if optogenetic stimulation is used
    task.useOpto = False

    task.sPlusDir = '1secNoise/Fig_left/Left_Movie_'
    task.sMinusDir = '1secNoise/Fig_right/Right_Movie_'

    # Stim and task params
    task.boundary = int(0)
    task.deviations = [10, 22.5, 45]
    if not task.useMovie:
        ...
        # Here we list the movie files. These are only used if task.useMovie
        # is True
    ...
    task.rewardPeriod = task.grayDuration  # no longer than gray duration!
