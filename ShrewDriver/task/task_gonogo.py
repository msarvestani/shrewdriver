    ...
    def check_state_progression(self):
        ...
        if self.state == States.SPLUS:
            ...
            if now > self.stateEndTime:
                ...
                #if self.doHint: #moved to reward period to account for delay
        ...
        if self.state == States.REWARD:
            ...
            # -- possibly dispense a small amount of reward for hint -- #
            if now >= self.stateStartTime + self.rewardDelay:
                if self.doHint:
                    self.training.dispense_hint(self.hintBolus / 1000)
                    self.doHint = False
